...
def error(words, label):
    ...
    if l is not 0:
        mpoint = len(words) - l
        pe = Pe(l)
        nerror = random.sample(range(mpoint, len(words)), pe if pe <= 3 else 3)
        for i in nerror:
            s = ["a", "b", "c", "d"]
            s.remove(words[i])
...
def main():
    ...
    while(True):
        ...
        printProgress(len(words_list), max_size, 'Progress', 'Complete')
        if(len(words_list) >= max_size):
            break

        result = S("S")
        words = list(result + result)
        if len(words) > N:
            continue
